...
(function () {
  ...
  function classifyGap(text) {
    ...
    if (/choose|decide|which|pick|option|between|vs\./.test(t))
      return "decision";
    if (
      /can\u2019?t|cannot|blocked|stuck|access|locked|won\u2019?t let/.test(t)
    )
      return "barrier";
    if (/role|supposed to|expected|how do i|new here|onboard/.test(t))
      return "role";
    if (/lost|no idea|where to start|overwhelm|disorient/.test(t))
      return "spinout";
    if (/clear|helpful|useful|inform|recommend|likely|nps|satisfy/.test(t))
      return "washout";
    ...
    return {
      ...
      diagnosis: [
        ...
        "Reads as a possible " +
          ...
          "-oriented practitioner gap picture — but the stop is not surfaced in the participant\u2019s own terms, so the data may not resolve cleanly.",
      ...
      diff: [
        ...
        {
          kind: "add",
          text: "Walk me through a specific moment when this came up for you.",
        },
        {
          kind: "add",
          text: "What were you trying to do, what did you reach for, and what got in the way?",
        },
      ],
  ...
  function sanitizeErrorMessage(err) {
    ...
    if (/Failed to fetch|NetworkError|Load failed/i.test(msg))
      return "network/CORS/preflight failure";
    if (/BAD_JSON|Could not parse response/i.test(msg))
      return "API returned invalid JSON";
    if (/HF_UNAUTHORIZED/i.test(msg))
      return "Hugging Face token/provider permission error";
    ...
    if (!res.ok || payload.ok === false) {
      const code =
        payload && payload.error && payload.error.code
          ...
          : "API_ERROR";
      const provider =
        payload && payload.meta && payload.meta.provider_message
          ? " — " + payload.meta.provider_message
          : "";
      throw new Error(
        "Rewriter API error: " + res.status + " " + code + provider,
      );
    }

    if (!payload.result)
      throw new Error("BAD_JSON: Rewriter API returned no result.");
    return {
      original: question,
      ...
      diff: payload.result.diff || [
        ...
        { kind: "add", text: payload.result.rewrite || "" },
      ],
  ...
  async function submitFeedback(result, type, comment) {
    const payload = {
      ...
      prompt_version:
        result.meta && result.meta.prompt_version
          ? result.meta.prompt_version
          : "",
  ...
  const EXAMPLES = [
    {
      ...
      text: "Did you find the pricing information clear?",
    },
    { label: "NPS", text: "How likely are you to recommend us to a friend?" },
    {
      label: "Adoption",
      text: "Why don\u2019t users adopt the new export feature?",
    },
    {
      label: "Engagement",
      text: "Do you have the information you need to do your job?",
    },
  ...
  window.initSMMRewriter = function (containerId) {
    ...
    const label = el("span", "rewriter-label", {
      text: "01 — Paste a research question",
    });
    ...
    const btnRun = el("button", "btn-primary-smm", {
      text: "Rewrite with SMM",
    });
    ...
    textarea.addEventListener("input", () => {
      btnRun.disabled = !textarea.value.trim();
    ...
    async function runRewrite() {
      ...
      currentRequestId =
        "smm-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7);
      ...
      try {
        ...
        if (local) {
          result = Object.assign({}, local, {
            source: "local_example",
            meta: { source: "local_example" },
          });
        } else {
        ...
        const errHint = el("div", "rewriter-label", {
          text: "Using offline heuristic mode. Reason: " + reason + ".",
        });
        errHint.style.cssText =
          "color:#7c2424;font-size:10px;margin-bottom:8px";
    ...
    btnCopy.onclick = () => {
      ...
      const md = [
        ...
        "",
        "**Diagnosis**",
        ...lastResult.diagnosis.map((d) => "- " + d),
        "",
        "**Rewrite**",
        "> " + lastResult.rewrite,
        "",
        "**Practitioner gap label:** " +
          (gap ? gap.label : lastResult.gap) +
          (gap ? " — " + gap.desc : ""),
        "",
        "**Why this works**",
        lastResult.why,
      ...
      navigator.clipboard.writeText(md).then(() => {
        ...
        setTimeout(() => {
          btnCopy.textContent = "Copy as markdown";
        }, 1800);
      });
    };
    ...
    function renderFeedback(r) {
      ...
      head.append(
        el("span", "step-n", { text: "06 · Feedback" }),
        el("span", "step-title", { text: "Help improve the tool" }),
      );

      const note = el("p", null, {
        text: "Feedback is optional. If you submit it, the original question, rewrite, label, and your comment are sent to the SMM API for review and may be stored in the feedback database. Do not include personal or sensitive information.",
      });
      note.style.cssText =
        "font-family:var(--serif);font-size:13px;line-height:1.5;color:var(--muted);margin:0 0 10px";
      ...
      comment.rows = 2;
      comment.placeholder =
        "What seems wrong? For example: label is wrong, rewrite is too leading, academic framing overstates Dervin, etc.";
      const submit = el("button", "btn-primary-smm", {
      ...
      submit.onclick = async () => {
        ...
        const fbType = selectedType || "issue";
        try {
          await submitFeedback(r, fbType, comment.value);
          status.textContent = "Thanks — feedback recorded.";
          form.style.display = "none";
          comment.value = "";
          if (window._smmTrack)
    ...
    function renderResult(r) {
      ...
      s1h.append(
        el("span", "step-n", { text: "02 · Diagnosis" }),
        el("span", "step-title", {
          text: "What\u2019s wrong with the question",
        }),
      );
      const s1list = el("ul", "diag-list");
      (r.diagnosis || []).forEach((d) => {
        const li = el("li", null, { text: d });
      ...
      s2h.append(
        el("span", "step-n", { text: "03 · Transformation" }),
        el("span", "step-title", { text: "Old → new" }),
      );
      const diffBlock = el("div", "diff-block");
      (r.diff || []).forEach((d) => {
        const line = el(
          "div",
          "diff-line " + (d.kind === "cut" ? "diff-cut" : "diff-add"),
        );
        line.append(
          el("span", "diff-mark", { text: d.kind === "cut" ? "–" : "+" }),
          el("span", null, { text: d.text }),
        );
      ...
      s3h.append(
        el("span", "step-n", { text: "04 · Rewrite" }),
        el("span", "step-title", { text: "SMM-neutral framing" }),
      );
      const rBox = el("div", "rewrite-box", {
        text: "\u201C" + r.rewrite + "\u201D",
      });
      const gapMeta = el("div", "gap-meta");
      const gapLabel = el("span", "rewriter-label", {
        text: "Practitioner label →",
      });
      ...
      gapTag.append(
        dot,
        document.createTextNode(" " + (gap ? gap.label : r.gap)),
      );
      const gapDesc = el("span", "gap-desc", { text: gap ? gap.desc : "" });
      const gapCaveat = el("span", "gap-desc", {
        text: "Tentative analytic prompt; return to the participant\u2019s own words before coding.",
      });
      ...
      whyP.style.cssText =
        "font-family:var(--serif);font-size:15px;line-height:1.6;color:var(--ink);margin:0";
      s4.append(s4h, whyP);
